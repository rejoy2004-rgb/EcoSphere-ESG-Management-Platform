...
import { buildReportDataset, ReportFilters } from '../utils/reporting';

const router = Router();

async function handleExport(res: Response, dataset: any, format: string, title: string, filters: ReportFilters) {
...
router.get('/environmental', authenticateJWT, async (req, res) => {
  ...
  const filters: ReportFilters = {
    ...
    dateRange: (start || end) ? { start: start as string, end: end as string } : undefined,
    module: 'ENVIRONMENTAL'
  };
  try {
    const dataset = await buildReportDataset(filters);
    await handleExport(res, dataset, format as string, 'Environmental (E) Report', filters);
  } catch (error: any) {
...
router.get('/social', authenticateJWT, async (req, res) => {
  ...
  try {
    const dataset = await buildReportDataset(filters);
    await handleExport(res, dataset, format as string, 'Social (S) Report', filters);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
});
...
router.get('/governance', authenticateJWT, async (req, res) => {
  ...
  try {
    const dataset = await buildReportDataset(filters);
    await handleExport(res, dataset, format as string, 'Governance (G) Report', filters);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
});

router.get('/esg-summary', authenticateJWT, async (req, res) => {
  const { departmentId, start, end, format } = req.query;
  const filters: ReportFilters = {
    departmentId: departmentId as string,
    dateRange: (start || end) ? { start: start as string, end: end as string } : undefined
  ...
  try {
    const dataset = await buildReportDataset(filters);
    await handleExport(res, dataset, format as string, 'ESG Comprehensive Summary Report', filters);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/custom', authenticateJWT, async (req, res) => {
  ...
  try {
    const dataset = await buildReportDataset(filters);
    await handleExport(res, dataset, format as string, 'Custom Dynamic ESG Report', filters);
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
});
