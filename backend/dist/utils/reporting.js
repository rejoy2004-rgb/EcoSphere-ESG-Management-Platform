...
exports.buildReportDataset = buildReportDataset;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
async function buildReportDataset(filters) {
    ...
    if (includeSoc) {
        ...
        if (start || end) {
            actWhere.date = {};
            if (start)
                actWhere.date.gte = start;
            if (end)
                actWhere.date.lte = end;
        }
        if (esgCategory) {
        ...
        if (start || end) {
            partWhere.createdAt = {};
            if (start)
                partWhere.createdAt.gte = start;
            if (end)
                partWhere.createdAt.lte = end;
        }
    ...
    if (includeGov) {
        ...
        if (start || end) {
            ...
            if (start)
                auditWhere.startDate.gte = start;
            if (end)
                auditWhere.startDate.lte = end;
        }
        ...
        if (start || end) {
            issueWhere.dueDate = {};
            if (start)
                issueWhere.dueDate.gte = start;
            if (end)
                issueWhere.dueDate.lte = end;
        }
    ...
    if (includeGam) {
        ...
        if (start || end) {
            chalWhere.createdAt = {};
            if (start)
                chalWhere.createdAt.gte = start;
            if (end)
                chalWhere.createdAt.lte = end;
        }
